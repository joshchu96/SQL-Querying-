...
SELECT category, COUNT(*) AS number_of_apps FROM analytics
GROUP BY category
ORDER BY number_of_apps desc;

--Find the top 5 most-reviewed apps and the number of reviews for each.
SELECT app_name, SUM(reviews) AS review_num FROM analytics
GROUP BY app_name
ORDER BY review_num DESC
LIMIT 5;

--Find the app that has the most reviews with a rating greater than equal to 4.8.
SELECT app_name, SUM(reviews) AS reviews FROM analytics
WHERE rating >= 4.8
GROUP BY app_name
...
SELECT app_name, reviews, min_installs / reviews AS proportion, min_installs
FROM analytics 
